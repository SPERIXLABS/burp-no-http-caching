...
class BurpHttpNoCaching: BurpExtension, ProxyRequestHandler {
    ...
    override fun handleRequestReceived(interceptedRequest: InterceptedRequest?): ProxyRequestReceivedAction {
        ...
        while (itr.hasNext()){
            val header = itr.next()
           if (httpRequest == null) {
               httpRequest = interceptedRequest?.withRemovedHeader(header.first)
               httpRequest = httpRequest?.withHeader(header.first, header.second)
           } else {
               httpRequest = httpRequest.withRemovedHeader(header.first)
               httpRequest = httpRequest?.withHeader(header.first, header.second)
           }
        }
